

import Foundation

// Question Value
let value = ""

extension String {
	func charAt(_ index: Int) -> Character {
		return self[self.index(self.startIndex, offsetBy: index)]
	}
	
	// Same as charAt, but wraps around the end of the string
	func circularCharAt(_ index: Int) -> Character {
		guard self.isEmpty == false else { fatalError("What did you expect calling this with an empty string?") }
		var realIndex = index
		if realIndex >= self.count {
			realIndex -= self.count
		}
		return charAt(realIndex)
	}
}

//The captcha requires you to review a sequence of digits (your puzzle input) and find the sum of all digits that match the next digit in the list. The list is circular, so the digit after the last digit is the first digit in the list.

//For example:

// 1122 produces a sum of 3 (1 + 2) because the first digit (1) matches the second digit and the third digit (2) matches the fourth digit.
// 1111 produces 4 because each digit (all 1) matches the next.
// 1234 produces 0 because no digit matches the next.
// 91212129 produces 9 because the only digit that matches the next one is the last digit, 9.

func calcSum(list: String) -> Int {
	guard list.isEmpty == false else { return 0 }
	
	// take the first character and append it to the end of the list
	let newList = "\(list)\(list.first!)"
	
	// Group pairs together
	var groupedList = ""
	var index = 0
	while (index < newList.count) {
		if index == newList.count-1 { break }
		let currentChar = newList.charAt(index)
		let nextChar = newList.charAt(index+1)
		if currentChar == nextChar {
			groupedList.append(currentChar)
		}
		index += 1	
	}
	return groupedList.reduce(0, {$0 + (String($1) as NSString).integerValue })
}
assert(calcSum(list: "1122") == 3)
assert(calcSum(list: "1111") == 4)
assert(calcSum(list: "1234") == 0)
assert(calcSum(list: "91212129") == 9)

// answer for part 1
print(calcSum(list: value))

// Part 2
//1212 produces 6: the list contains 4 items, and all four digits match the digit 2 items ahead.
//1221 produces 0, because every comparison is between a 1 and a 2.
//123425 produces 4, because both 2s match each other, but no other digit has a match.
//123123 produces 12.
//12131415 produces 4.

func calcSumPart2(list: String) -> Int {
	guard list.isEmpty == false else { return 0 }
	let pivot = list.count / 2
	
	// Group pairs together
	var groupedList = ""
	var index = 0
	while (index < list.count) {
		if index >= list.count { break }
		let currentChar = list.circularCharAt(index)
		let nextChar = list.circularCharAt(index+pivot)
		if currentChar == nextChar {
			groupedList.append(currentChar)
		} 
		index += 1	
	}
	return groupedList.reduce(0, {$0 + (String($1) as NSString).integerValue })
}

assert(calcSumPart2(list: "1212") == 6)
assert(calcSumPart2(list: "1221") == 0)
assert(calcSumPart2(list: "123425") == 4)
assert(calcSumPart2(list: "123123") == 12)
assert(calcSumPart2(list: "12131415") == 4)

// answer for part 2
print(calcSumPart2(list: value))

