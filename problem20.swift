...
final class Point: Comparable {
  let number: Int
  var position: [Int]
  var velocity: [Int]
  let acceleration: [Int]
  
  init(number: Int, position: [Int], velocity: [Int], acceleration: [Int]) {
    self.number = number
  ...
  func tick() {
    (0...2).forEach { self.velocity[$0] += self.acceleration[$0] }
    (0...2).forEach { self.position[$0] += self.velocity[$0] }
  }
  
  func distance() -> Int {
    return self.position.map { abs($0) }.reduce(0, +)
  }
...
func pointArray(from: String) -> [Point] {
  var points = [Point]()
  var lines = from.split(separator: "\n")
  let regex = try! NSRegularExpression(pattern: "(-?\\d+)")
  for i in 0..<lines.count {
    let line = lines[i]
    let range = NSRange((line.startIndex..<line.endIndex), in: line)
    let matches = regex.matches(in: String(line), range: range).map { match in 
      ((String(line) as NSString).substring(with: match.range) as NSString).integerValue
    }
    let point = Point(
      number: i,
      position: Array(matches[0...2]),
      velocity: Array(matches[3...5]),
      acceleration: Array(matches[6...8]))
    points.append(point)
  }
  return points
}

func calcPart1(points: [Point]) {
  let tickCount = 400 // arbitrary!
  for _ in 0..<tickCount {
    points.forEach { $0.tick() }
  }
  print("Part1: \(points.sorted().first!.number)")
}
let p = pointArray(from: input)
calcPart1(points: p)

func calcPart2(points: [Point]) {
  var points = points
  let tickCount = 500 // arbitrary!
  for _ in 0..<tickCount {
    points.forEach { $0.tick() }
    
    var pointDictionary = [String: [Point]]()
    for point in points {
      let hash = "\(point.position)"
      pointDictionary[hash, default: [Point]()].append(point)
    }  
    points = [Point]()
    for (_,v) in pointDictionary where v.count == 1 {
      points.append(v[0])
    }
  }
  print("Part2: \(points.count)")
}

let p2 = pointArray(from: input)
calcPart2(points: p2)
