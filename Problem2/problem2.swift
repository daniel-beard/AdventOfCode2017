...
func difference(list: [Int]) -> Int {
	guard let min = list.min(), let max = list.max() else { fatalError("No min or max") }
	return max - min
}

let rows = value.split(separator: "\n").map({ $0.split(separator: "\t").map({ ($0 as NSString).integerValue }) })
let answer = rows.map({ difference(list: $0 )}).reduce(0, +)
print(answer)
...
func evenDivisorsResult(list: [Int]) -> Int {
	// O(n^2), yo
	for a in list {
		for b in list {
			if a == b { continue }
...
let a = rows.map({ evenDivisorsResult(list: $0) }).reduce(0, +)
print(a)