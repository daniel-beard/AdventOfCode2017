...
extension String {
    ...
    mutating func pop(_ val: Int) -> String {
        let result = String(self.prefix(val))
        self = String(self.dropFirst(val))
        return result
    }
}

func takeUntilEndOfGarbage(_ input: String) -> (clean: String, garbage: String) {
    var input = String(input.dropFirst()) // eat opening '<'
    var garbage = ""
    while true {
        let currChar = String(input.prefix(1))
        if currChar == "!" {
            _ = input.pop(2)
        } else if currChar == ">" {
            _ = input.pop(1)
...
func cleanGarbage(_ input: String) -> (clean: String, garbage: String) {
    var input = input
    var result = ""
    var garbageResult = ""
    while true {
        let currChar = String(input.prefix(1))
        guard !currChar.isEmpty else { break }
        if currChar == "<" {
            let (clean, garbage) = takeUntilEndOfGarbage(input)
            input = clean
            garbageResult += garbage
        } else {
            result += input.pop(1)
        }
    }
    return (result, garbageResult)
}
...
func scoreGroups(_ input: String) -> Int {
    ...
    while true {
        ...
        if currChar == "{" {
            stack.append("{")
            score += stack.count
        } else if currChar == "}" && !stack.isEmpty {
            stack.removeLast(1)
        }
        _ = input.pop(1)
    }
...
let (cleanedInput, garbage) = cleanGarbage(input)
print("Part 1: \(scoreGroups(cleanedInput))")
print("Part 2: \(garbage.count)")
