...
class ConditionalExpr {
	...
	init(registerName: String, comparison: ComparisonType, value: Int) {
	...
	func evaluate(registers: [String: Int]) -> Bool {
		let register = registers[registerName, default: 0]
		switch comparison {
			case .lt: return register < value
			case .gt: return register > value
			case .eq: return register == value
			case .ne: return register != value
			case .gte: return register >= value
			case .lte: return register <= value
		}
	}
}
...
class IncrementExpr {
	var registerName: String
	var exprType: AdditionType
	var value: Int
	var conditional: ConditionalExpr
	
	init(registerName: String, exprType: AdditionType, value: Int, conditional: ConditionalExpr) {
		self.registerName = registerName
	...
	func evaluate(registers: [String: Int]) -> [String: Int] {
		guard conditional.evaluate(registers: registers) else { return registers }
		var registers = registers
		var register = registers[registerName, default: 0]
		switch exprType {
			case .inc: register += value
			case .dec: register -= value
		}
...
class Driver {
	...
	// b inc 5 if a > 1
	func parseLine(_ line: String) -> IncrementExpr {
		var line = line
		let registerName = line.take(until: " ")
		line.drop(1) // eat space
		let incType = AdditionType(rawValue: line.take(until: " "))!
		line.drop(1) // eat space
		let value = (line.take(until: " ") as NSString).integerValue
		line.drop(1) // eat space
		line.drop(2) // eat 'if'
		line.drop(1) // eat space
		let conditionalRegister = line.take(until: " ")
		line.drop(1) // eat space
		let comparisonType = ComparisonType(rawValue: line.take(until: " "))!
		line.drop(1) // eat space
		let conditionalValue = (line as NSString).integerValue
		
		let conditionalExpr = ConditionalExpr(
			registerName: conditionalRegister, 
			comparison: comparisonType, 
			value: conditionalValue
		)
		let incrementExpr = IncrementExpr(
