...
// Part 1
let lines = phrases.split(separator: "\n")
let regex = try! NSRegularExpression.init(pattern: "\\b([a-zA-Z]+)\\b.*\\b(\\1)\\b", options: [])
let validPhrases = lines.filter { regex.numberOfMatches(in: String($0), options: [], range: NSRange(location: 0, length: String($0).count)) == 0 }
...
assert(checkPassphrase("abcde fghij"))
assert(checkPassphrase("abcde xyz ecdab") == false)
assert(checkPassphrase("a ab abc abd abf abj"))
assert(checkPassphrase("iiii oiii ooii oooi oooo"))
assert(checkPassphrase("oiii ioii iioi iiio") == false)
print(lines.filter({ checkPassphrase(String($0)) }).count)